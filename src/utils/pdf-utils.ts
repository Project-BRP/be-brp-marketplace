...
export class PDFUtils {
  static async createInvoice(transaction: IGetTransactionResponse): Promise<string> {
    ...
    const logoFile = fs.readdirSync(uploadsPath).find(file => /\.(png|jpeg)$/i.test(file));
    ...
    doc
      ...
      .text('Telp: (021) 12345678 | Email: info@bumirekayasa.com', { align: 'right' })
    ...
    const status = transaction.deliveryStatus ?? transaction.manualStatus ?? '-';
    const tanggalFormatted = DateTime.fromISO(transaction.createdAt instanceof Date ? transaction.createdAt.toISOString() : transaction.createdAt).setZone('Asia/Jakarta').toFormat("dd MMMM yyyy, HH:mm");
    ...
    doc
      ...
      .text(`Metode Pembayaran: ${transaction.paymentMethod || '-'}`, { align: 'left' })
      .text(`Alamat Pengiriman: ${transaction.shippingAddress || '-'}`, { align: 'left' })
      .moveDown(1.5);
    ...
    const tableRight = doc.page.width - doc.page.margins.right; // 595.28 - 50 = 545.28
    const totalTableWidth = tableRight - col1; // 495.28
    ...
    const colWidths = {
      ...
      qty: 50,
      unitPrice: 110,
      total: 90, // dikurangi dari 100 agar tidak mepet kanan
    };
    ...
    doc
      ...
      .text('Harga Satuan', col4, tableTop + 5, { width: colWidths.unitPrice, align: 'right' })
      .text('Total', col5, tableTop + 5, { width: colWidths.total, align: 'right' });

    let y = tableTop + 20;
    const items = transaction.transactionItems.filter(item => !item.isStockIssue);
    ...
    items.forEach((item, index) => {
      ...
      const textHeight = doc.font('Helvetica').fontSize(10).heightOfString(`${productName}${packagingName}`, { width: textWidth });
      const rowHeight = Math.max(20, textHeight + 10);

      if (index % 2 === 0) {
        doc.fillColor(LIGHT_GRAY).rect(col1, y, totalTableWidth, rowHeight).fill();
      }
      ...
      doc
        ...
        .text(`${productName}${packagingName}`, col2, y + (rowHeight / 2 - 4), { width: textWidth })
        .text(item.quantity.toString(), col3, y + (rowHeight / 2 - 4), { width: colWidths.qty, align: 'right' })
        .text(`Rp${item.priceRupiah.toLocaleString('id-ID')}`, col4, y + (rowHeight / 2 - 4), { width: colWidths.unitPrice, align: 'right' })
        .text(`Rp${totalPrice.toLocaleString('id-ID')}`, col5, y + (rowHeight / 2 - 4), { width: colWidths.total, align: 'right' });
    ...
    const addTotalLine = (
      ...
      doc
        ...
        .text(`Rp${value.toLocaleString('id-ID')}`, valueX, currentY, { width: lineWidth, align: 'right' });

      doc.moveDown(0.4);
    };

    addTotalLine('Subtotal:', transaction.cleanPrice);
    addTotalLine(`PPN (${transaction.PPNPercentage || 0}%):`, transaction.priceWithPPN - transaction.cleanPrice);
    ...
    addTotalLine('TOTAL:', transaction.totalPrice, { bold: true, color: THEME_COLOR });
    doc.moveDown(1);
